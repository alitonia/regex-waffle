...
SIZE = 5000000  # essentially +inf


def readFile(filename):
	with open(os.path.join('sv_data/req/', filename), 'r') as f:
		match = [word for word in f.readline().split(", ")]
		# removing of new line from last word in M
		match[-1] = match[-1][:-1]
		unmatch = [x for x in filter(lambda x: x.__len__() > 0, [word for word in f.readline().split(", ")])]
...
def generate_regex(filename: str):
	...
	match, unmatch = (
		[x.query for x in frac_match if len(x.query) > 0 and x.query != '/'],
		[x.query for x in frac_unmatch if len(x.query) > 0 and x.query != '/'],
	)
	...
	def starter(match, unmatch):
		...
		def genetic_programming(match, unmatch):
			...
			for i in range(GENERATIONS_NUM):
				...
				for j in range(ELITIZM_SIZE, POPULATION_SIZE, 2):
					...
					crossover(population[parent1Index], population[parent2Index], newPopulation[j],
					          newPopulation[j + 1])
		...
		return treeToString(last_top.code)
